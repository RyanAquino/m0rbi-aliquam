...
$spStatus = "SELECT username FROM sp WHERE BINARY username = '$unameReject'"; 
$sp_statusQuery = mysqli_query($connect, $spStatus);
$sp_statusCount = mysqli_num_rows($sp_statusQuery);

$cStatus = "SELECT username FROM client WHERE BINARY username = '$unameReject'";
...
$c_statusCount = mysqli_num_rows($c_statusQuery);
		...
		if($sp_statusCount > 0){
			$updateReject = "UPDATE sp SET reg_status = 'reject' where BINARY username = '$unameReject'";
		...
		if($c_statusCount > 0){
			$updateReject = "UPDATE client SET reg_status = 'reject' where BINARY username = '$unameReject'";
			$updateRejected = mysqli_query($connect, $updateReject);
			echo "<div class='alertsuccess'>"; 
                echo "<p>User rejected</p>";
                echo "</div>";
