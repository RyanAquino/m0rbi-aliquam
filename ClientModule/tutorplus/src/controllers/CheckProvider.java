...
public class CheckProvider extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String sql3 = "SELECT sched_id,day,time FROM schedule NATURAL JOIN sp where firstname = '" + tutName + "'";
			st = conn.createStatement();
			rs = st.executeQuery(sql3);
			while(rs.next()) {
				schedule sched = new schedule();
				sched.setSchedId(rs.getInt("sched_id"));
				sched.setDay(rs.getString("day"));
				sched.setTime(rs.getString("time"));
				scList.add(sched);
			}
			
			int myId = spInf.getId();
			
			schList = scList;
			request.setAttribute("schedList",schList);
			request.setAttribute("name", name);
			request.setAttribute("lastname", lastname);
			
			request.setAttribute("servId", myId);
